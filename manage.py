...
@click.option("--minago",prompt="modified time before now",help="modified time before now",default=60*3,type=int)
@click.pass_context
def newcontributor(context,name,email,website,minago):
    """ 
    ...
    """
    cmd = ["find",'./',"\\(",'-name',"\'*.py\'","\\)","-a","-mmin",str(0-minago),"-exec","sed","-i",
...
@click.option("--minago",prompt="modified time before now",help="modified time before now",default=60*3,type=int)
@click.pass_context
def newmodifydate(context,minago):
    """
    ...
    cmd = ['find','./','\\(','-name',"\'*.py\'",'\\)','-mmin',str(0-minago),
...
@click.option("--minago",prompt="modified time before now",help = "modified time before now",default=60*3,type=int)
@click.pass_context
def modifydate(context,minago):
    """
        change the modified date on all the file which you have modified by in the past minago to NOW()
    """
    import datetime 
