...
@click.option('--delete-time', default=3* 60,
...
@click.option('--scheduler-cls', default='weblocust.scheduler.ThreadBaseScheduler', callback=load_cls,
...
@click.option('--fetcher-cls', default='weblocust.fetcher.Fetcher', callback=load_cls,
...
@cli.command()
@click.option('--processor-cls', default='weblocust.processor.Processor',
...
@click.option('--webui-instance', default='weblocust.webui.app.app', callback=load_cls,
...
def phantomjs(ctx, phantomjs_path, port, auto_restart, args):
    ...
    phantomjs_fetcher = os.path.join(
        os.path.dirname(weblocust.__file__), 'fetcher/phantomjs_fetcher.js')
    cmd = [phantomjs_path,
...
def bench(ctx, fetcher_num, processor_num, result_worker_num, run_in, total, show,
    ...
    """
    from weblocust.libs import bench
    from weblocust.webui import bench_test  # flake8: noqa
    ...
    try:
        ...
        for i in range(result_worker_num):
            threads.append(run_in(ctx.invoke, result_worker,
                                  result_cls='weblocust.libs.bench.BenchResultWorker',
        ...
        for i in range(processor_num):
            threads.append(run_in(ctx.invoke, processor,
                                  processor_cls='weblocust.libs.bench.BenchProcessor',
                                  **processor_config))
        ...
        for i in range(fetcher_num):
            threads.append(run_in(ctx.invoke, fetcher,
                                  fetcher_cls='weblocust.libs.bench.BenchFetcher',
                                  **fetcher_config))
        ...
        threads.append(run_in(ctx.invoke, scheduler,
                              scheduler_cls='weblocust.libs.bench.BenchScheduler',
                              **scheduler_config))
...
def one(ctx, interactive, enable_phantomjs, scripts):
    ...
    if g.resultdb is None:
        result_worker_config.setdefault('result_cls',
                                        'weblocust.result.OneResultWorker')
    ...
    scheduler_config.setdefault('scheduler_cls',
                                'weblocust.scheduler.OneScheduler')
